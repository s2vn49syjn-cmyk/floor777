...
(async()=>{
 ...
 try{
  ...
  await fill('numberCount',3);page.once('dialog',d=>d.accept());await page.locator('#assign').click();await saved();assert.equal(await page.locator('.seat').count(),3);assert.equal(await page.locator('#fieldConfirmed').isChecked(),false);await page.locator('#undo').click();await saved();assert.equal(await page.locator('.seat').count(),5);
  await page.locator('#homeMode').click();
  const seat=page.locator('.seat').first(),oldX=Number(await seat.getAttribute('x')),bb=await seat.boundingBox();await page.mouse.move(bb.x+bb.width/2,bb.y+bb.height/2);await page.mouse.down();await page.mouse.move(bb.x+bb.width/2+24,bb.y+bb.height/2+16,{steps:4});await page.mouse.up();await saved();assert.notEqual(Number(await page.locator('.seat').first().getAttribute('x')),oldX);await page.locator('#undo').click();await saved();
  await page.locator('#seatMove').check();const sb=await page.locator('.seat').first().boundingBox();await page.mouse.move(sb.x+sb.width/2,sb.y+sb.height/2);await page.mouse.down();await page.mouse.move(sb.x+sb.width/2+12,sb.y+sb.height/2+12,{steps:3});await page.mouse.up();await saved();assert.equal(await page.locator('#shape').inputValue(),'custom');await page.locator('#undo').click();await saved();await page.locator('#seatMove').uncheck();
  await page.locator('[data-add=arc]').click();await fill('count',3);await fill('x',750);await fill('y',400);await page.locator('#confirmed').check();await page.locator('#applyGeometry').click();await saved();
 ...
 }catch(e){if(page){fs.mkdirSync(path.join(root,'test-results'),{recursive:true});await page.screenshot({path:path.join(root,'test-results/failure.png'),fullPage:true}).catch(()=>{});fs.writeFileSync(path.join(root,'test-results/failure.txt'),String(e)+'\n'+await page.content());}throw e;}finally{await browser?.close();server.kill();}
