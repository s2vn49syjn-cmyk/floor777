...
(async()=>{
 ...
 await page.locator('[data-screen=map]').click();
 assert.equal(await page.locator('.floor-bg').evaluate(el=>getComputedStyle(el).fill),'rgb(255, 255, 255)');
 assert.match(await page.locator('.seat[data-seat="562"]').evaluate(el=>getComputedStyle(el).animationName),/floor-pick-blink/);
 await page.emulateMedia({reducedMotion:'reduce'});
 assert.equal(await page.locator('.seat[data-seat="562"]').evaluate(el=>getComputedStyle(el).animationName),'none');
